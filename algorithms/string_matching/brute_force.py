...
"""
In this algorithm, we start by comparing the characters of the pattern string and the text string one by one, and if all the characters 
of the pattern are matched with the text, we return the index position of the text where the first character of the pattern is located.
If any character of the pattern is mismatched with the text string, we shift the pattern by one position to check if the pattern appears 
at the next index position. We continue comparing the pattern and text string by shifting the pattern by one index position.
"""
...
def brute_force(text, pattern):
    ...
    while i < l1:         # iterating from the 0th index of text
        ...
        if count == l2:   # it shows a matching of pattern in the text 
                ...
                flag = True
                # return the starting index of the successful match
                return i
        i += 1
    if not flag: 
